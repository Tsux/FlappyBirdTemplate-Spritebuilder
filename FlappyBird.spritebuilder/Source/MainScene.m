...
- (void)didLoadFromCCB {
    ...
    //Some Considerations:
    //only move layer on x axis, so just x values for parallex are changing
    //bush ratio snould be lower than clouds cause clouds are farther away from bush
    //the parallax effect must most affect nearby objects than the farthest
...
- (void)touchBegan:(UITouch *)touch withEvent:(UIEvent *)event {
    if (!_gameOver) {
        [character.physicsBody applyAngularImpulse:9000.f];
        _sinceTouch = 0.f;
...
- (void)update:(CCTime)delta
{
    ...
    //loop the clouds
    for(CCNode *cloud in _clouds){
        //get the worlds positions of the cloud
        ...
        /*This loop was used to move the bushes individually
        //move the cloud
        cloud.position = ccp(cloud.position.x - (character.physicsBody.velocity.x * delta), cloud.position.y);
        
        //if the left corner is one complete width off the screen, move it the to the right
    ...
    //loop the bushes
    for(CCNode *bush in _bushes){
        //get the worlds position of the bush
        ...
        /* This loop was used to move the clouds individually
        //move the bush
